...
- **用户人设**：人设名、描述、注入设置连同头像一起备份，逐个勾选。下载是合并进本机，你自己的人设不会被删掉。
- **API 配置**：连接配置档逐个勾选，可以连它引用的 API 密钥与代理密码一起带上，换台机器不用重新填一遍。
...
人设在酒馆里没有自己的文件 —— 名字、描述、注入设置都存在 `settings.json` 里，`User Avatars/` 目录里只有头像图片。插件把这几个字段抽出来拼成一个 `personas.json` 参与备份，头像跟着一起传。

菜单里逐个勾选人设，只有勾中的那几个会被抽出来。默认人设本身也被勾中时，"哪个是默认"才一起带上。

从云端下载时是**合并**：同名人设用云端版本覆盖，本机独有的人设留着不动，`settings.json` 里其余的内容一个字都不碰。下载完人设面板当场就变，不用刷新页面。
...
**勾中的配置档会连它引用的 API 密钥明文与代理密码一起备份**，换台机器不用重新填一遍。这些密钥是明文写进网盘上的 `api-profiles.json` 的，介意就别勾这一类 —— 其余五类都不会读取密钥。没有被勾中的配置档引用到的密钥一个都不碰。
...
│  └─ <世界书名>.json
├─ API配置/
│  └─ api-profiles.json            连接配置档，含它引用到的密钥明文
└─ .st-sync/
...
用户人设与 API 配置是例外。它们存在 `settings.json` 与 `secrets.json` 里，下载时**逐键合并**：同一个人设、同一个配置档用云端版本覆盖，本机独有的留着，这两个文件里其余的内容不动。
...
- **API 密钥只在你勾了「API 配置」之后才会离开本机**，而且是明文写进网盘上的 `api-profiles.json`。只有被勾中的那几个配置档引用到的密钥会被带上，`secrets.json` 里其余的密钥不会被读取，也不会整份上传。不勾这一类，密钥不出本机。
